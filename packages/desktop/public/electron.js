...
function createWindow() {
  // Completely disable application menu before creating window
  ...
  // Make sure menu is removed in all ways possible
  mainWindow.setMenuBarVisibility(false);
  mainWindow.removeMenu();
  
  // Expose menu hiding to the renderer process
  ipcMain.handle('hide-menu', () => {
    mainWindow.setMenuBarVisibility(false);
    mainWindow.removeMenu();
  });

  // Prevent keyboard shortcuts from showing menu bar
  mainWindow.on('focus', () => {
    mainWindow.setMenuBarVisibility(false);
  });
  ...
  mainWindow.once('ready-to-show', () => {
    mainWindow.show();
    mainWindow.setMenuBarVisibility(false); // Make sure menu is hidden when window shows
  });
...
// When Electron is ready, create window and ensure menu is hidden
app.whenReady().then(() => {
  Menu.setApplicationMenu(null); // Ensure menu is removed app-wide
  createWindow();
});
